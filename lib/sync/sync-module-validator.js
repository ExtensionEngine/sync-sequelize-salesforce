...
const { ConsumableError, DomainError, FkError, UniquenessError } = errors;
...
class SyncModuleValidator {
  ...
  validateDomain() {
    this.instances.forEach(inst => {
      inst.validateDomain().forEach(it => {
        const error = new DomainError(inst, it);
        this.syncModule.errors.add(error);
      });
    });
  }
}
