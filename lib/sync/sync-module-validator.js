...
class SyncModuleValidator {
  ...
  get dependencies() {
    const query = { options: { consumable: true }, isSelfAssociation: false };
  ...
  async run(callPath) {
    try {
      await this.validateDependencies(callPath);
  ...
  validateDependencies(callPath) {
    return Promise.each(this.dependencies, it => it.validate([...callPath]));
  }
