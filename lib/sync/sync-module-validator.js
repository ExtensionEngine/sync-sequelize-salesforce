...
class SyncModuleValidator {
  ...
  async run(callPath) {
    try {
      ...
      return this.syncModule.errorManager.invalidate({ model: this.Model });
    } catch (err) {
  ...
  pushError(...args) {
    return this.syncModule.errorManager.add(...args);
  }

  hydrate(associations) {
    associations = [].concat(associations);
    // TODO: we need all attributes for domain check, but only id and is_valid for
    // consumables check - potentially revise if too consuming
