...
Arbiter.prototype.configure = function (config) {
  const { connection = {}, sfLogger, sfQueryLogger, ...otherConfig } = config;
  configure.call(this, { ...otherConfig, connection: omit(connection, 'oauth2') });
  const conn = getConnection(this.oracle);
  conn.oauth2 = new jsforce.OAuth2({
    ...connection.oauth2,
    loginUrl: connection.loginUrl
  });
  setLogger(conn, sfLogger, sfQueryLogger);
  return this.oracle;
};
